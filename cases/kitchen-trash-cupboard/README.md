# Kitchen Trash Cupboard CAD Free-Tool Benchmark

無料でローカル実行できる CAD-as-code 系ツールを使い、同じ「下部にゴミ箱を3つ置けるキッチンカップボード」仕様を生成して比較するためのベンチマークです。

現時点では `CadQuery`、`build123d`、`JSCAD`、`OpenSCAD`、`ForgeCAD CLI` を同じ仕様データから生成し、CAD/メッシュ出力、寸法検証、スクリーンショット、レポートをまとめます。

## 確認した前提

- このケースの作業ディレクトリは `cases\kitchen-trash-cupboard` です。
...
cd <repo-root>
uv sync --python 3.11
cd cases\kitchen-trash-cupboard
npm install
...
- 比較動画のソース: [videos/kitchen-trash-cupboard-comparison/](videos/kitchen-trash-cupboard-comparison/)
...
```powershell
cd <repo-root>\cases\kitchen-trash-cupboard
uv run scripts/run_all.py
```
...
- 本体外形（上部扉/取手の前方突出を除く）: 幅 1680mm × 奥行 650mm × 高さ 1800mm
- 全体 bbox（上部扉/取手の前方突出込み）: 幅 1680mm × 奥行 698mm × 高さ 1800mm
- 蹴込み: 90mm
- 下部ゴミ箱ベイ: 3列、1列あたり有効幅 約536mm × 有効奥行 590mm × 有効高さ 712mm
- ゴミ箱プレースホルダー: 幅 440mm × 奥行 500mm × 高さ 620mm を3台
- 板厚: 側板/天板/底板/棚板 18mm、背板 6mm、扉 16mm、カウンター区切り 24mm
- 構成: フルハイト側板、下部3分割オープンベイ、ゴミ箱3台、上部棚収納、上部両開き扉、取手、蹴込み
...
HyperFrames 版の比較動画ソースは [videos/kitchen-trash-cupboard-comparison/](videos/kitchen-trash-cupboard-comparison/) にあります。現在の構成は日本語版で、ページ遷移をゆっくり見られるよう 82秒 / 30fps にしています。
...
```powershell
cd videos\kitchen-trash-cupboard-comparison
