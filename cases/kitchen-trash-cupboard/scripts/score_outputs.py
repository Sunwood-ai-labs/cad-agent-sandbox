...
def score_candidate(manifest: dict[str, object], output_dir: Path) -> dict[str, object]:
    ...
    feature_points = 0
    feature_points += 7 if role_counts.get("carcass", 0) >= 7 else 0
    feature_points += 5 if role_counts.get("divider", 0) >= 3 else 0
    feature_points += 5 if role_counts.get("shelf", 0) >= 4 else 0
    feature_points += 4 if role_counts.get("door") == 2 else 0
    feature_points += 3 if role_counts.get("handle") == 2 else 0
    feature_points += 6 if role_counts.get("trash_bin") == 3 else 0
    feature_points += 2 if role_counts.get("trash_lid") == 3 else 0
