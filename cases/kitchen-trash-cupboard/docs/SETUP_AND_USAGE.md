...
このメモは、次回以降に `cases\kitchen-trash-cupboard` でゴミ箱3台対応キッチンカップボード CAD ベンチを再開するための手順書です。Windows + PowerShell + `uv` 前提で書いています。
...
寸法や部品構成を変える場合は、まず `cupboard_benchmark\spec.py` を変更します。現行仕様は幅1680mm、奥行650mm、高さ1800mm、下部ゴミ箱3台分のオープンベイを前提にしています。
